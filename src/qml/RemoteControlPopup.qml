...
Popup {
    ...
    contentItem: ColumnLayout {
        ...
        ColumnLayout {
            ...
            Text { text: qsTr("Connect Tablet"); color: Theme.text; font.pixelSize: 17; font.weight: Font.DemiBold }
            Text {
                text: qsTr("Scan this code from the PWA to pair a remote device.")
        ...
        // Fase (PWA UX/security refactor): lets the user shut the web
        // server down entirely (it previously ran unconditionally for the
        // app's whole lifetime) - persisted via PwaServer::serverEnabled,
        // so the choice survives a restart too.
        RowLayout {
            Layout.fillWidth: true
            Layout.leftMargin: Theme.spacingLg
            Layout.rightMargin: Theme.spacingLg
            ...
            ToggleSwitch {
                checked: pwaServer.serverEnabled
                onToggled: (v) => pwaServer.serverEnabled = v
            }
        }

        Text {
            visible: !pwaServer.serverEnabled
            Layout.fillWidth: true
            Layout.leftMargin: Theme.spacingLg
            Layout.rightMargin: Theme.spacingLg
            Layout.topMargin: Theme.spacingSm
            Layout.bottomMargin: Theme.spacingMd
            text: qsTr("Web Dashboard is Offline. Turn it on to connect your phone.")
            color: Theme.subtext0
            font.pixelSize: 12
            font.italic: true
            wrapMode: Text.WordWrap
        ...
        ColumnLayout {
            ...
            ColumnLayout {
                ...
                Text {
                    Layout.alignment: Qt.AlignHCenter
                    text: qsTr("PIN: ") + pwaServer.securityToken
                    color: Theme.text
                    font.pixelSize: 22
                ...
                ToolButton {
                    Layout.alignment: Qt.AlignHCenter
                    label: qsTr("⟳ Renew PIN")
                    onClicked: pwaServer.regenerateSecurityToken()
                }
            }
            Text {
                Layout.alignment: Qt.AlignHCenter
